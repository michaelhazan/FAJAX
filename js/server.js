class Server {
  ...
  processMessage(message) {
    ...
    if (this.functions[messageType]) {
      ...
      message.requester = [
        message.responder,
        (message.responder = message.requester),
      ][0];
  ...
  #put(message) {
    ...
    switch (body.type) {
      case "edit":
        ...
        if (!body.item.text || body.item.marked === null)
          throw `Incorrect item type! (not a TodoItem).`;
...
class UsersServer extends Server {
  ...
  #generateUserID() {
    ...
    for (
      let i = 0;
      i < this.#userIDLength - amountOfUsers.toString().length;
      i++
    ) {
